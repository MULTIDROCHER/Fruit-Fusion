...
public class ZippyWater2D : MonoBehaviour {
	...
	[Tooltip("Enable / Disable creating wave colliders.\nCannot be changed during runtime.")]
	public bool createCollders = true;
	...
	void Init() {
		...
		waveColliders = new GameObject[0];
		if (createCollders) {
		...
		for (int i = 0; i < pointAmount; i++) {
			...
			accY[i] = 0;
			velY[i] = 0;
			if(createCollders && i < colliderAmount) {
	...
	void Wave() {
		...
		for (int i = 0; i < colliderAmount; i++) velY[i] +=h * Mathf.Sin((Time.time * waveSpeed) + i * f);
	}

	void Noise() {
		if (noise <= 0) return;
		for (int i = 0; i < colliderAmount; i++) 	velY[i] += Random.Range(-noise, noise);
	}

	void RandomSplash() {
		if (randomSplash <= 0) return;
		if (Time.frameCount % randomSplashDelay != 0) return;
		int r = Random.Range(0, colliderAmount -2);
